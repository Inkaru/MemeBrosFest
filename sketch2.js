urls = [
  ...
  function setup() {
    ...
    last_i = -1
    for (let row = 0; row < nrow + 1; row++) {
        for (let col = 0; col < ncol + 3; col++) {
            i = Math.floor((Math.random() * imgs.length ));
            if(last_i == i) i = (i +1) % imgs.length
            last_i = i

            icons.push(new Icon(imgs[i], col * padding + (row % 2) * offset, row*padding))
        }
      }
  ...
  function draw() {
    ...
    icons.forEach(element => {
        image(element.image, element.x,element.y)
        element.x += speedX
        element.y += speedY

        //if(element.x < 0 - emote_size) element.x = element.x + canvas_X
        //if(element.y < 0 - emote_size) element.y = element.y + canvas_Y
        if(element.x > canvas_X + (emote_size + padding)) element.x = element.x - canvas_X - (emote_size + padding)*3.5/2
        if(element.y > canvas_Y ) element.y = element.y - canvas_Y - (emote_size + padding) / 2
    });

    logo.resize(600,0)
    image(logo, canvas_X/2 - logo.width/2,canvas_Y/2 - logo.height/2)
